...
public class KrakenMovement : MonoBehaviour
{
    ...
    [SerializeField] private float decelerationSpeed = 4.0f;
    [SerializeField] private float movementChargeTime = 4.0f;
    ...
    void LateUpdate()
    {
        playerDetected = Vector3.Distance(target.transform.position, transform.position) < hostileDetectionDistance && !playerDetected ? true : false;
    ...
    IEnumerator StraightAttack()
    {
        ...
        while (attackChargeTimePassed <= movementChargeTime)
        {
            DecelerateByFrame();
